...
def AIkernel(
    ...
    def neural(smiles_list, batch_size=batch_size, n_jobs=n_jobs):
        ...
        class NN(torch.nn.Module):
            ...
            def forward(self, inputs):
                ...
                # gruA = self.activation(gruA)
                # gruB = self.activation(gruB)
                # gruC = self.activation(gruC)
                # gruD = self.activation(gruD)

                fcA = self.fcA(gruA)
                # fcA = self.activation(fcA)
                fcB = self.fcB(gruB)
                # fcB = self.activation(fcB)
                fcC = self.fcC(gruC)
                # fcC = self.activation(fcC)
                fcD = self.fcD(gruD)
                # fcD = self.activation(fcD)
        ...
        return predictions(nn_ox, x, device, batch_size=batch_size), predictions(
            nn_red, x, device, batch_size=batch_size
        )
    ...
    if return_smiles:
        temp = smiles_list
        rr = []
        rr.append(np.intersect1d(temp, smi + exceptions, return_indices=True)[1])
        while rr[-1].size > 0:
